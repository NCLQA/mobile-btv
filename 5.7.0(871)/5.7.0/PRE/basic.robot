...
56669 카테고리 구성 확인
    #1. "탐색" > 카테고리 내 "TV 방송" 진입    /    1. "TV 방송" 타이틀 노출 확인
    ...
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.view.ViewGroup[@content-desc="TV 방송버튼"]
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Wait Until Element Is Visible    Xpath=//android.widget.TextView[@text="TV 방송"]
    Press Keycode    4
    Sleep    1
...
56620 ZEM 검색어 자동완성
    ...
    Sleep    1
    #자동완성 목록 / 자동완성 목록은 편성 변경 시 변경되므로 확인 후 변경 후 자동화 검증
    Wait Until Element Is Visible    Xpath=//android.widget.TextView[@text="뽀로로와 노래해요"]
    Wait Until Element Is Visible    Xpath=//android.widget.TextView[@text="뽀롱뽀롱 뽀로로"]
    Wait Until Element Is Visible    Xpath=//android.widget.TextView[@text="뽀로로 슈퍼슈퍼 쇼"]
    Wait Until Element Is Visible    Xpath=//android.widget.TextView[@text="뽀로로"]
    Wait Until Element Is Visible    Xpath=//android.widget.TextView[@text="뽀로로 극장판 보물섬 대모험"]
    Wait Until Element Is Visible    Xpath=//android.widget.TextView[@text="뽀로로 최고의 순간"]
...
56625 ZEM 키즈 종료
    ...
    FOR  ${i}  IN RANGE    0    10    #My oksusu 소장 VOD 이용하기 노출시점까지 스크롤
        Swipe By Percent    50    70    50    30
        ${pass}    Run Keyword And Return Status    Wait until Element Is Visible    Xpath=//android.widget.TextView[@text="My oksusu 소장 VOD 가져오기"]
        Exit For Loop If    ${pass}
    END
    Sleep    1
    ...
    END
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.ScrollView/android.view.View[4]
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skb.smartrc:id/dialog_input
    Sleep    1
    성인인증번호(1111) 입력
...
56763 월정액 카드_상품탐색 카드 전체 상품 보기 이동
    #1. 앱 실행 >  "홈" > 퀵메뉴 내 "콘텐츠이용권" 진입
    #2. [전체 상품 보기] 버튼 선택    /    2. '전체 상품 보기' 타이틀, 월정액 블록 노출 확인
    #3. 상단 이전(<) 버튼 선택    /    3. '월정액' 타이틀 노출 확인 시 pass 

    #FOR  ${i}  IN RANGE    0    10    #월정액 퀵메뉴 노출시점까지 스크롤 / 퀵매뉴 편성이 하단으로 변경되면 사용
        #Swipe By Percent    50    70    50    30
        #${pass}    Run Keyword And Return Status    Wait until Element Is Visible    Xpath=(//android.widget.ImageView[@resource-id="com.skb.smartrc:id/iv_icon"])[2]
        #Exit For Loop If    ${pass}
    #END
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=(//android.widget.FrameLayout[@resource-id="com.skb.smartrc:id/card_view"])[2]    #콘텐츠이용권 진입
    Sleep    1
    ...
    Wait Until Keyword Succeeds    1 min    3 sec    Wait Until Element Is Visible    Xpath=(//android.view.ViewGroup[@resource-id="com.skb.smartrc:id/cl_root"])[1]    #콘텐츠 이용권 블록 노출 확인
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.ImageButton[@content-desc="이전"]
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Wait Until Element Is Visible    Xpath=//android.widget.TextView[@text="콘텐츠 이용권"]
...
56674 앱 구동 후 공개예정/VOD 콘텐츠 자동재생 시 음소거로 재생 확인
    ...
    #1. "탐색" > 임의의 VOD 검색 후 진입    /    2. 음소거상태 로그(isMuted = true) 출력 시 pass / 공개예정 편성종료로 삭제
...
56727 시놉시스 > 패키지 시놉시스 > 구조
    ...
    #2. 미구매 패키지 조커 1 & 2 시리즈 패키지 [HD] 진입    /    2. 패키지 시놉시스 구조(이미지, 구매버튼, 구성콘텐츠, 콘텐츠 시놉시스) 노출 확인 시 pass

    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.view.ViewGroup[@content-desc="탐색버튼"]
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.view.ViewGroup[@content-desc="영화/시리즈버튼"]
    Sleep    1
    FOR  ${i}  IN RANGE    0    20    #할인&패키지 노출시점까지 스크롤
        Swipe By Percent    50    80    50    30
    ...
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.ImageView[@content-desc="베놈 전편 소장 패키지 [HD]"]    #편성 변경이 되므로 변경 후 검증 스크롤하여 하단 콘텐츠는 진입 전 영화/시리즈와 겹쳐서 인식 불가
...
85638 홈 footer > 개인정보 이용내역 조회 메뉴 추가
    ...
    FOR  ${i}  IN RANGE    0    20    #최하단으로 스크롤
        Swipe By Percent    50    80    50    30
        ${pass}    Run Keyword And Return Status    Wait until Element Is Visible    com.skb.smartrc:id/tv_skbroadband
        Exit For Loop If    ${pass}
    END
    Element Should Be Visible    com.skb.smartrc:id/tv_privacy
    Element Should Be Visible    com.skb.smartrc:id/tv_privacy_history
    Element Should Be Visible    com.skb.smartrc:id/tv_terms
...
85639 홈 footer > 개인정보 이용내역 선택
    ...
    FOR  ${i}  IN RANGE    0    20    #최하단으로 스크롤
        Swipe By Percent    50    80    50    30
        ${pass}    Run Keyword And Return Status    Wait until Element Is Visible    com.skb.smartrc:id/tv_privacy_history
        Exit For Loop If    ${pass}
    END
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skb.smartrc:id/tv_privacy_history
    Sleep    1
...
57083 댓글/리뷰 작성 기능 제공 - 댓글 작성 정책-1
    ...
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skb.smartrc:id/et_search_bar
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Input Text    com.skb.smartrc:id/top_search_bar    미생
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.TextView[@text ="미생"]
    Sleep    2
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.view.ViewGroup[@content-desc="미생"]
    Sleep    1
...
57287 댓글/리뷰 작성 기능 제공 - 댓글 수정/삭제 정책-1
    ...
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=(//android.view.View[@resource-id="com.skb.smartrc:id/v_comment_more"])[2]
    Sleep    1
...
57242 댓글/리뷰 작성 기능 제공 - 신고-2
    ...
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skb.smartrc:id/btn_comment    #댓글남기기 버튼 클릭
    Sleep    1
        FOR  ${i}  IN RANGE    0    10    #타인의 댓글 노출까지 스크롤
        Swipe By Percent    50    70    50    30
        ${pass}    Run Keyword And Return Status    Wait until Element Is Visible    Xpath=(//android.view.View[@resource-id="com.skb.smartrc:id/v_comment_more"])[4]
        Exit For Loop If    ${pass}
    END
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=(//android.view.View[@resource-id="com.skb.smartrc:id/v_comment_more"])[4]
    Sleep    2
...
57223 MY 프론트 정책 - 답글 있는 댓글 삭제
    ...
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.TextView[@text="삭제하기"]
    Sleep    1
    Element Should Be Visible    Xpath=//android.widget.TextView[@text="답글을 삭제하시겠어요?"]
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skb.smartrc:id/btn_left
    Sleep    1
    Page Should Not Contain Element    Xpath=//android.widget.TextView[@text="답글을 삭제하시겠어요?"]    #팝업 종료 확인
    ...
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skb.smartrc:id/btn_right
    Sleep    1
    Page Should Not Contain Element    Xpath=//android.widget.TextView[@resource-id="com.skb.smartrc:id/tv_comment" and @text="11:11"]    #댓글 삭제 확인

57177 MY - 내 댓글 메뉴
    #1. MY > 하단 스크롤 후 댓글 목록 진입 > 내 댓글 타이틀 확인
    ...
    END
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.TextView[@resource-id="com.skb.smartrc:id/tv_menu_name" and @text="댓글 목록"]
    Page Should Contain Element    Xpath=//android.widget.TextView[@resource-id="com.skb.smartrc:id/title" and @text="내 댓글"]