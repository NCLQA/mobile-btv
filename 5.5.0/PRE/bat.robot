...
#56532 무료 VOD 재생 확인
    ...
    #Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skb.smartrc:id/et_search_bar
    #Sleep    1
    #Wait Until Keyword Succeeds    1 min    3 sec    Input Text    com.skb.smartrc:id/top_search_bar    웰컴투 삼달리
    #Sleep    1
    #Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.TextView[@text ='웰컴투 삼달리']
    #Sleep    2
    #Run Process    adb logcat -c    shell=True
    #Run Keyword And Ignore Error    Remove File    D://RFA/log.txt

    #Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.view.ViewGroup[@content-desc="웰컴투 삼달리"]
    #Sleep    90
    #${log}    Run Process    adb logcat -d 80 >> D://RFA/log.txt    shell=True
...
56533 패키지 게이트웨이 진입 확인
    #0. 검증 콘텐츠 : 미구매 패키지 <위키드 자막+더빙 소장 패키지>, 구매 패키지 <엘리멘탈 자막+더빙 소장 패키지>
    #1. 앱 실행 > "탐색" > "영화/시리즈" > "할인&패키지" 진입
    #2. 미구매 패키지 <위키드 자막+더빙 소장 패키지> 진입    /    2. 구매버튼 노출 확인
    #3. 뒤로가기
    ...
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.ImageView[@content-desc="위키드 자막+더빙 소장 패키지 [HD]"]
...
116633 AI Smart Controller - TV 컨트롤러 전원
    #0. STB 버전 5.4.3 이상 연결
    #1. 앱 실행 > Top 메뉴 리모컨 선택 > 전원 버튼 선택
    #2. 팝업 노출 확인 / 2. 10초 뒤 전원 버튼 선택으로 STB 켬
    ...
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skb.smartrc:id/btn_power
    Run Process    adb logcat -c    shell=True
    Run Keyword And Ignore Error    Remove File    D://RFA/log.txt
