...
월정액 화면 내 기간권 이용중 문구 노출 확인
    ...
    FOR  ${i}  IN RANGE    0    10    #월정액 퀵메뉴 노출시점까지 스크롤
        Swipe By Percent    50    70    50    30
        ${pass}    Run Keyword And Return Status    Wait until Element Is Visible    Xpath=//android.widget.FrameLayout[@resource-id="com.skb.smartrc:id/card_view" and @index="0"]
        Exit For Loop If    ${pass}
    END
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.FrameLayout[@resource-id="com.skb.smartrc:id/card_view" and @index="0"]
    Sleep    1
...
월정액 카드_상품탐색 카드 전체 상품 보기 이동
    ...
    FOR  ${i}  IN RANGE    0    10    #월정액 퀵메뉴 노출시점까지 스크롤
        ...
        Exit For Loop If    ${pass}
    END
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.FrameLayout[@resource-id="com.skb.smartrc:id/card_view" and @index="0"]    #월정액 진입
    Sleep    1
