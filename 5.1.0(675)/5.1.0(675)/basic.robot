...
56665 Navigation Back_예외케이스 확인 (인물정보)
    ...
    Sleep    1
    Swipe By Percent    50    70    50    30
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.view.ViewGroup[@content-desc="암살"]
    Sleep    1
    Wait Until Element Is Visible    Xpath=//android.widget.TextView[@text = "암살"]
...
56645 월정액 화면 내 기간권 이용중 문구 노출 확인
    ...
    FOR  ${i}  IN RANGE    0    10    #월정액 퀵메뉴 노출시점까지 스크롤
        Swipe By Percent    50    70    50    30
        ${pass}    Run Keyword And Return Status    Wait until Element Is Visible    Xpath=(//android.widget.ImageView[@resource-id="com.skb.smartrc:id/iv_icon"])[1]
        Exit For Loop If    ${pass}
    END
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=(//android.widget.ImageView[@resource-id="com.skb.smartrc:id/iv_icon"])[1]
    Sleep    1
...
56620 ZEM 검색어 자동완성
    ...
    Wait Until Element Is Visible    Xpath=//android.widget.TextView[@text="뽀롱뽀롱 뽀로로 8"]
...
56763 월정액 카드_상품탐색 카드 전체 상품 보기 이동
    ...
    FOR  ${i}  IN RANGE    0    10    #월정액 퀵메뉴 노출시점까지 스크롤
        ...
        Exit For Loop If    ${pass}
    END
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=(//android.widget.ImageView[@resource-id="com.skb.smartrc:id/iv_icon"])[1]    #월정액 진입
    Sleep    1
...
56663 Navigation Back_GNB
    #1. 앱 실행 > 홈, 탐색, AI PICK, 무료, Free 채널 각각 이전 키 1회 입력    /    '앱 종료 안내' 토스트 팝업 로그(toast) 출력 및 각 페이지 메뉴 노출 확인

    Wait Until Element Is Visible    Xpath=//android.widget.RelativeLayout[@content-desc="홈버튼"]
    Run Process    adb logcat -c    shell=True
    Run Keyword And Ignore Error    Remove File    D://RFA/log.txt
    Press Keycode    4    #이전 키
    Sleep    1
    ...
    Should Contain    ${String}    Toast
    Sleep    1
    Page Should Contain Element    Xpath=//android.widget.RelativeLayout[@content-desc="홈버튼"]
    
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.RelativeLayout[@content-desc="탐색버튼"]
    Sleep    1
    Wait Until Element Is Visible    Xpath=//android.widget.RelativeLayout[@content-desc="탐색버튼"]
    Run Process    adb logcat -c    shell=True
    Run Keyword And Ignore Error    Remove File    D://RFA/log.txt
    Press Keycode    4    #이전 키
    Sleep    1
    ...
    Should Contain    ${String}    Toast
    Sleep    1
    Page Should Contain Element    Xpath=//android.widget.RelativeLayout[@content-desc="탐색버튼"]

    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.RelativeLayout[@content-desc="AI PICK버튼"]
    Sleep    1
    Wait Until Element Is Visible    Xpath=//android.widget.RelativeLayout[@content-desc="AI PICK버튼"]
    Run Process    adb logcat -c    shell=True
    Run Keyword And Ignore Error    Remove File    D://RFA/log.txt
    Press Keycode    4    #이전 키
    Sleep    1
    ...
    Should Contain    ${String}    Toast
    Sleep    1
    Page Should Contain Element    Xpath=//android.widget.RelativeLayout[@content-desc="AI PICK버튼"]

    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.RelativeLayout[@content-desc="무료버튼"]
    Sleep    1
    Wait Until Element Is Visible    Xpath=//android.widget.RelativeLayout[@content-desc="무료버튼"]
    Run Process    adb logcat -c    shell=True
    Run Keyword And Ignore Error    Remove File    D://RFA/log.txt
    Press Keycode    4    #이전 키
    Sleep    1
    ...
    Should Contain    ${String}    Toast
    Sleep    1
    Page Should Contain Element    Xpath=//android.widget.RelativeLayout[@content-desc="무료버튼"]

    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.RelativeLayout[@content-desc="Free채널버튼"]
    Sleep    1
    Wait Until Element Is Visible    Xpath=//android.widget.RelativeLayout[@content-desc="Free채널버튼"]
    Run Process    adb logcat -c    shell=True
    Run Keyword And Ignore Error    Remove File    D://RFA/log.txt
    Press Keycode    4    #이전 키
    Sleep    1
    ...
    Should Contain    ${String}    Toast
    Sleep    1
    Page Should Contain Element    Xpath=//android.widget.RelativeLayout[@content-desc="Free채널버튼"]

56727 시놉시스 > 패키지 시놉시스 > 구조
    #1. 앱 실행 > "탐색" > "영화/시리즈" > "할인&패키지" 진입
    #2. 미구매 패키지 <트랜스포머 전편 소장 패키지> 진입    /    2. 패키지 시놉시스 구조(이미지, 구매버튼, 구성콘텐츠, 콘텐츠 시놉시스) 노출 확인 시 pass
...
85740 모비소식 웹페이지 이동규격 inlink로 변경 - 퀵 메뉴
    #1. 앱 실행 > "홈" > 퀵메뉴 내 "모비소식" 선택    /    1. "모비소식" 타이틀 확인 시 pass 

    FOR  ${i}  IN RANGE    0    10    #모비소식 퀵메뉴 노출시점까지 스크롤
        Swipe By Percent    50    70    50    30
        ${pass}    Run Keyword And Return Status    Wait until Element Is Visible    Xpath=(//android.widget.FrameLayout[@resource-id="com.skb.smartrc:id/card_view"])[5]
        Exit For Loop If    ${pass}
    END
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=(//android.widget.FrameLayout[@resource-id="com.skb.smartrc:id/card_view"])[5]
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Wait until Element Is Visible    Xpath=//android.widget.TextView[@text="모비 소식"]
...
56896 지상파 콘텐츠 시놉시스 내 안내문구 수정
    ...
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skb.smartrc:id/et_search_bar
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Input Text    com.skb.smartrc:id/top_search_bar    태어난 김에 세계일주3
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.TextView[@text ='태어난 김에 세계일주3']
    Sleep    2
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.view.ViewGroup[@content-desc="태어난 김에 세계일주3"]
    Sleep    1
    FOR  ${i}  IN RANGE    0    3   
        Swipe By Percent    50    70    50    30
    END
    Element Should Be Visible    Xpath=//android.widget.TextView[@resource-id="com.skb.smartrc:id/tv_message" and @text="지상파 콘텐츠의 경우 B tv에서만 시청 가능한 회차는\n제공되지 않습니다. 지상파 월정액에 가입하시면\n모든 회차를 모바일에서 시청하실 수 있습니다.\n\n※ 일부 콘텐츠는 방송사, 콘텐츠 제공사 또는 저작권자의 요청에 의해\n모바일 시청이 제공되지 않을 수 있습니다."]
...
57242 댓글/리뷰 작성 기능 제공 - 신고-2
    ...
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skb.smartrc:id/btn_comment    #댓글남기기 버튼 클릭
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=(//android.view.ViewGroup[@resource-id="com.skb.smartrc:id/cl_item_body"])[6]/android.view.View
    Sleep    2
...
57223 MY 프론트 정책 - 답글 있는 댓글 삭제
    ...
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=(//android.view.ViewGroup[@resource-id="com.skb.smartrc:id/cl_item_body"])[2]/android.view.View[1]    #더보기 선택
    Sleep    1
